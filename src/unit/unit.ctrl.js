(function () {
  angular.module("archer").controller("UnitCtrl", unitCtrl);
  ...
  function unitCtrl($scope, $routeParams, $timeout, $location) {
    ...
    function localData() {
      ...
      local.currentWidth =
        (($(window).height() - $(".flip-control").height()) * 922) / 600;
    ...
    function localFunctions() {
      ...
      local.otherBooks = function () {
        return _.remove(_.cloneDeep(books), function (book) {
          if (local.currentUnit.relevantBooks) {
            return (
              book.id !== _.toNumber($routeParams.bookId) &&
              _.includes(local.currentUnit.relevantBooks, book.id)
            );
          } else {
      ...
      local.determineBookVisibility = function () {
        ...
        _.each($scope.currentBook.unitMapping, function (unit) {
          if (
            _.toNumber(unit.pageStart) <= $scope.currentPage &&
            _.toNumber(unit.pageEnd) >= $scope.currentPage
          ) {
            local.currentUnit = unit;
            _.each(local.currentUnit.lessons, function (page, lesson) {
              let nextLessonPage = local.currentUnit.lessons[
                _.toNumber(lesson) + 1
              ]
                ? local.currentUnit.lessons[_.toNumber(lesson) + 1]
                : unit.pageEnd;
              if (
                _.toNumber(page) <= _.toNumber($scope.currentPage) &&
                _.toNumber($scope.currentPage) <= _.toNumber(nextLessonPage)
              ) {
        ...
        _.each($scope.otherBooks, function (book) {
          ...
          if (currentOtherUnit) {
            if (
              !currentOtherUnit.lessons[_.toNumber(local.currentLesson)] &&
              !($scope.currentBook.id === 2 && book.id === 3)
            ) {
              book.disabled = true;
            } else {
              book.disabled = false;
    ...
    function scopeFunction() {
      ...
      $scope.goToPage = function (to) {
        ...
        //The following logic will break if a unit is missing a video or if the videos per unit change from 2 to sth else it also assumes that all videos correspond to lesson 4
          ...
          let nextPage =
            Number($scope.pageId) < $scope.currentBook.totalPages
              ? Number($scope.pageId) + 1
              : Number($scope.pageId);
          if (nextPage % 2 == 0) {
            $location.path(
              "/unit/" + $scope.bookId + "/" + $scope.unitId + "/" + nextPage
            );
          }
        } else if (to === "prev") {
          let prevPage =
          ...
          if (prevPage % 2 != 0) {
            $location.path(
              "/unit/" + $scope.bookId + "/" + $scope.unitId + "/" + prevPage
            );
          }
        }
      };
        ...
        _.each($scope.currentBook.unitMapping, function (unit) {
          _.each(unit.pageMapping, function (page) {
            if (page.id === _.toNumber(currentPage)) {
              if (page.videoId) {
                $location.path(
                  "/unit/6/" + Math.ceil(page.videoId / 2) + "/" + page.videoId
                );
              } else {
                ...
                $location.path("/unit/1/" + unitID + "/" + Transcript);
              }
            }
          });
        });
        ...
        if ($scope.currentBook.id === 2 && otherBook.id === 3) {
          ...
          $location.path(
            "/unit/" +
              ...
              otherUnit.id +
              "/" +
              [_.toNumber(Object.values(otherUnit.lessons)[0])]
          );
        } else if (
          $scope.currentBook.id === 3 &&
          otherBook.id === 2 &&
          localStorage.getItem("wbLastPage") != 0
        ) {
          console.log("working");
          console.log(localStorage.getItem("wbLastPage"));
          $location.path(
            "/unit/" +
              ...
              otherUnit.id +
              "/" +
              localStorage.getItem("wbLastPage")
          );
          localStorage.setItem("wbLastPage", 0);
        } else if (
          $scope.currentBook.id === 3 &&
          ...
          $location.path(
            "/unit/" +
              ...
              otherUnit.id +
              "/" +
              otherUnit.lessons[_.toNumber(lessonNumber)]
          );
        }
      };
      ...
      $scope.openPath = function (page, path) {
        ...
        if (path.zoomOverride) {
          $scope.zoomWindow.url =
            "images/books/img/" +
            $scope.currentBook.prefixImg +
            $scope.convertIndexToPage(page) +
            "OverRide.jpg";
        } else {
          $scope.zoomWindow.url =
            "images/books/img/" +
        ...
        $scope.zoomWindow.height =
          ((path.zoomOverride ? path.zoomOverride.height : path.height) * 600) /
          local.currentHeight;
        $scope.zoomWindow.left =
          (-(path.zoomOverride ? path.zoomOverride.left : path.left) * 461) /
          (local.currentWidth / 2);
        $scope.zoomWindow.width =
          ((path.zoomOverride ? path.zoomOverride.width : path.width) * 461) /
          (local.currentWidth / 2);
        ...
        $scope.currentAnswers = path.zoom
          ? _(path.zoom.answers)
              .map()
              .flatMap(function (o) {
                return o.paths;
        ...
        $timeout(function () {
          if (
            $("#zoomBox")[0].getBoundingClientRect().height >
            local.currentHeight - $(".flip-control").height()
          ) {
            $scope.zoomWindow.imgDivHeight = $scope.zoomWindow.height + "px";
            $scope.zoomWindow.height =
              local.currentHeight / 2.1 - $(".flip-control").height();
          ...
          var updateScrollPos = function (e) {
            ...
            $("#zoomBox").scrollTop(
              $("#zoomBox").scrollTop() + (clickY - e.pageY)
            );
          };
        ...
        _.each(pagePaths, function (object) {
          if (!object.widthDif) {
            ...
            if (object.zoomOverride) {
              object.zoomOverride.width =
                (object.zoomOverride.width * local.currentWidth) /
                2 /
                (922 / 2);
            }
          ...
          if (!object.leftDif) {
            ...
            if (object.zoomOverride) {
              object.zoomOverride.left =
                (object.zoomOverride.left * (local.currentWidth / 2)) /
                (922 / 2);
            }
            object.leftDif = true;
          }
          ...
          if (!object.heightDif) {
            ...
            if (object.zoomOverride) {
              object.zoomOverride.height =
                (object.zoomOverride.height * local.currentHeight) / 600;
            }
      ...
      $scope.nextPath = function (direction) {
        ...
        if (
          ...
          ($scope.currentBook.id === 1 ||
            ($scope.currentBook.id === 3 && $scope.currentPage !== 12)) &&
          ($scope.currentPage % 2 === 0 ||
            ($scope.currentBook.id === 3 &&
              ($scope.currentPage === 11 || $scope.currentPage === 13)))
        ) {
          ...
          nextIndex === paths.length &&
          ($scope.currentBook.id === 1 || $scope.currentBook.id === 3) &&
          $scope.currentPage % 2 === 1
        ) {
          //nextIndex = indexOfPath - direction;
        ...
        // }
          ...
          nextIndex < 0 &&
          ($scope.currentBook.id === 1 ||
            ($scope.currentBook.id === 3 && $scope.currentPage !== 13)) &&
          ($scope.currentPage % 2 === 1 ||
            ($scope.currentBook.id === 3 &&
              ($scope.currentPage === 12 || $scope.currentPage === 14)))
        ) {
          $scope.currentPage -= 1;
          paths = $scope.locatePagePaths($scope.currentPage);
      ...
      $scope.showUnderline = function (answer) {
        if (answer.underline) {
          ...
          let underlinePaths = _($scope.currentAnswers)
            .filter({ id: underlineId, type: "underline" })
            .value();
      ...
      $scope.answerAction = function (answer) {
        ...
        if (answer.type === "checkmark") {
          $scope.showUnderline(answer);
      ...
      $scope.openDebug = function () {
        ...
        canvas.onmousemove = function (e) {
          ...
          if (element !== null) {
            element.style.width =
              Math.abs(local.mouse.x - local.mouse.startX) + "px";
            element.style.height =
              Math.abs(local.mouse.y - local.mouse.startY) + "px";
            element.style.left =
              local.mouse.x - local.mouse.startX < 0
                ? local.mouse.x + "px"
                : local.mouse.startX + "px";
            element.style.top =
              local.mouse.y - local.mouse.startY < 0
                ? local.mouse.y + "px"
                : local.mouse.startY + "px";
          }
        };
        ...
        canvas.onclick = function (e) {
          if (element !== null) {
            ...
            p.innerHTML =
              "top:" +
              _.replace(element.style.top, "px", "") +
              ",<br/>left:" +
              _.replace(element.style.left, "px", "") +
              ",<br/>width:" +
              _.replace(element.style.width, "px", "") +
              ",<br>height:" +
              _.replace(element.style.height, "px", "");
      ...
      $scope.startPainting = function () {
        ...
        if ($scope.zoomWindow.drawingMode) {
          ...
          $("#canvasDrawer").attr(
            "width",
            $("#zoomBox").width() * $scope.zoomWindow.scale
          );
          $("#draftDrawer").attr(
            "width",
            $("#zoomBox").width() * $scope.zoomWindow.scale
          ...
          $("#canvasContainer").height(
            $("#zoomBox")[0].getBoundingClientRect().height + 20
          );
          if ($scope.zoomWindow.overflowY == "scroll") {
            $("#canvasDrawer").attr(
              "height",
              _.toNumber(_.replace($scope.zoomWindow.imgDivHeight, "px", "")) *
                $scope.zoomWindow.scale
            );
            $("#draftDrawer").attr(
              "height",
              _.toNumber(_.replace($scope.zoomWindow.imgDivHeight, "px", "")) *
          ...
          $("#canvasDrawer").mousedown(function (e) {
            ...
            var mouseY =
              e.pageY -
              $("#canvasContainer").offset().top +
              $("#canvasContainer").scrollTop();
            paint = true;
            addClick(mouseX, mouseY);
            local.redraw();
            if (local.curTool !== "rect" && local.curTool !== "text")
              local.undoHistory.push({
                type: "path",
                obj: local.undoIndexPath++,
              });
          });
          ...
          $("#canvasContainer").on("scroll", function () {
            $("#zoomBox").scrollTop(
              $(this).scrollTop() * $scope.zoomWindow.scale
            );
          });
          ...
          local.redraw = function () {
            ...
            function drawRect(currentRect, index) {
              draft.beginPath();
              var width =
                currentRect.x[currentRect.x.length - 1] - currentRect.x[0];
              var height =
                currentRect.y[currentRect.y.length - 1] - currentRect.y[0];
            ...
            for (var i = 0; i < local.clickX.length; i++) {
              ...
              if (isMarker || isEraser) {
                ...
                draft.globalCompositeOperation =
                  local.globalCompositeOperation[i];
                draft.globalAlpha = 1;
            ...
            for (var i = 0; i < local.clickX.length; i++) {
              var isMarker = local.curToolArray[i] === "marker";
              var isRect =
                local.curToolArray[i] === "rect" ||
                local.curToolArray[i] === "text";
              var obj = {};
              if (local.clickDrag[i] && i) {
                obj.moveX = local.clickX[i - 1];
                obj.moveY = local.clickY[i - 1];
      ...
      $timeout(function () {
        ...
        local.oTurn.bind("turning", function (event, page, view) {
          if (page === 1 /*|| page === $scope.currentBook.totalPages*/) {
            ...
            $(".containerFlip").css(
              "left",
              $(window).width() / 2 - local.currentWidth / 2
            );
          }
        });
        ...
        local.oTurn.bind("turning", function (event, page, view) {
          ...
          let endPage =
            $scope.currentBook.unitMapping[
              $scope.currentBook.unitMapping.length - 1
            ].pageEnd;
