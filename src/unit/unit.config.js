    ...
    function config($routeProvider) {
      $routeProvider.when("/unit/:bookId/:unitId/:pageId?", {
        template:
          ...
          "style=\"top:{{((answer.type == 'multipleCircle') ? answer.top:((answer.draggable) ? answer.top-10 : answer.top-4))}}px;left:{{answer.left}}px;width: {{answer.width}}px;height: {{(answer.multiline) ? 25:((answer.type == 'multipleCircle') ? (answer.height ? answer.height : 9):12)}}px; transform:rotate({{answer.rotate ? answer.rotate : 0}}deg);\"" +
